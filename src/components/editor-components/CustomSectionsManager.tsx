...
const CustomSectionsManager: React.FC<CustomSectionsManagerProps> = ({
  ...
  const addCustomSectionBlock = (sectionId: string) => {
    ...
    if (section) {
      // ✅ FIX: Ensure blocks array exists
      if (!section.blocks) {
        section.blocks = [];
      }
  ...
  const removeCustomSectionBlock = (sectionId: string, blockId: string) => {
    const newData = { ...data };
    const section = newData.customSections.find(s => s.id === sectionId);
    if (section && section.blocks) {  // ✅ FIX: Check blocks exists
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-6">
        {data.customSections.map((customSection) => {
          // ✅ FIX: Ensure blocks array exists before rendering
              ...
              <div className="space-y-3">
                ...
                {blocks.length === 0 ? (
                  ...
                  <DndContext
                    ...
                    onDragEnd={(e) => onDragEnd(e, data, onUpdate, 'customSection', customSection.id)}
                  >
                    <SortableContext
